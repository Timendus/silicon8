...
export default (instance) => {
  ...
  window.addEventListener("gamepadconnected", (e) => {
    ...
    if (e.gamepad.mapping != "standard") {
      ...
      return console.log(
        `🎮 Ignoring gamepad ${e.gamepad.id} because it is not a 'standard mapping' gamepad`
      );
    }
  ...
  window.addEventListener("gamepaddisconnected", (e) => {
    ...
    if (unannouncedGamepads.includes(e.gamepad.index))
      unannouncedGamepads = unannouncedGamepads.filter(
        (g) => g != e.gamepad.index
      );
    // Ignore controllers that are not in our main list
    if (!gamepads[e.gamepad.index]) return;
    ...
    for (const gamepad of currentGamepads) {
      ...
      for (const key in keys) {
        if (gamepad.buttons[key].pressed != gamepads[gamepad.index][key]) {
          if (gamepad.buttons[key].pressed) instance.pressKey(keys[key]);
          else instance.releaseKey(keys[key]);
        }
      }
  ...
  function saveState(gamepad) {
    gamepads[gamepad.index] = Object.keys(keys)
